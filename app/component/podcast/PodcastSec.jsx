...
export const PostcastSec = () => {
 ...
 speaker: Array.isArray(data?.speakers) && data.speakers.length > 0
  ...
  const filteredResources = useMemo(() => {
    return resources.filter((item) => activeFilters.Industry === "All" || item.industry === activeFilters.Industry);
  ...
  const search = (value) => {
    const filtered = value
      ? resources.filter((r) => r.title.toLowerCase().includes(value.toLowerCase()))
      : resources;
    setCurrentPage(0);
    return filtered;
  };
  ...
  return (
    <section className="text-black px-4 py-10 bg-white min-h-screen overflow-x-hidden">
      <div className="container mx-auto w-full px-4 sm:px-6 lg:px-8">
              ...
              >
                ...
                <div className="w-full h-2/5 p-4 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between my-2 h-1/4">
                    <ButtonLayout text="Read More" hoverImage={ButtonImage} className="!h-[40px] !w-[150px]" />
        ...
        <AnimatePresence>
          {selectedVideo && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-md bg-black/60">
              <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} className="bg-white rounded-lg overflow-hidden w-11/12 max-w-3xl relative shadow-2xl">
                <button
                  onClick={() => {
                ...
                <div className="relative pt-[56.25%]">
                  {!showForm ? (
                    <ReactPlayer
                      ...
                      onProgress={(state) => {
                        if (state.playedSeconds >= 30 && !pausedAt30 && !completedForms[selectedVideo]) {
                          setPlaying(false);
                          setPausedAt30(true);
                          setResumeTime(state.playedSeconds);
                        }
                      }}
                      config={{ youtube: { playerVars: { start: resumeTime } } }}
                    />
                  ) : (
                    <div id="hubspot-form-container" className="absolute top-0 left-0 w-full h-full bg-white flex justify-center items-center p-6" />
                  )}
                </div>

                {pausedAt30 && !showForm && !completedForms[selectedVideo] && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <button onClick={() => setShowForm(true)} className="bg-[#2E3092] text-white px-6 py-3 rounded-lg shadow-lg hover:bg-[#1e2170] transition">
                      Tap to Continue
                    </button>
                  </div>
                )}
