...
export const DataSheetCards = () => {
  ...
  // Fetch datasheet and filters
  useEffect(() => {
    dispatch(fetchDatasheetList());
    dispatch(fetchBlogFilterList());
  }, [dispatch]);

  // Map datasheet data once it's loaded
  useEffect(() => {
  ...
  const selectFilter = (type, selected) => {
    ...
    if (type === "Topics") {
      ...
      if (exists) updatedFilters.Topics = updatedFilters.Topics.filter((t) => t._id !== selected._id);
      else {
    ...
    } else updatedFilters[type] = selected;
  ...
  const currentPageData = filteredResources.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
  ...
  return (
    <section className="text-black px-4 py-10 bg-white min-h-screen overflow-x-hidden">
      <div className="container mx-auto">
        ...
        {topicLimitWarning && <p className="text-red-500 text-sm mb-2">You can select a maximum of 3 topics.</p>}
              ...
              >
                ...
                <div className="absolute inset-0 bg-[#2E3092]/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-sm">
                  <div className="flex justify-between items-start px-4 w-full">
                    <h3 className="text-sm md:text-[16px] font-semibold w-9/12 break-words text-white">{item.title}</h3>
                    <div className="flex flex-col">
                      <button onClick={() => handleCopy(item.link, item.id)} className="text-white hover:text-black" title="Copy link">
                ...
                <div className="w-full px-4 py-3 flex flex-col justify-between group-hover:opacity-0 transition-opacity duration-300">
                  <div className="flex flex-wrap gap-2 mb-5 mt-3">
                    {item.tags.map((tag) => <span key={tag} className="bg-[#FF9F56] text-black text-xs px-2 py-1 rounded">{tag}</span>)}
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <div className=" mt-8 gap-[2px] rounded-2xl border border-gray-300 overflow-hidden select-none">
          <motion.button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 0} className={`px-4 py-3 bg-white border-r border-gray-300 ${currentPage === 0 ? "text-gray-400 cursor-not-allowed" : "text-[#2E3092]"}`}>
            <FaLessThan className="w-3 h-3"/>
          </motion.button>
          {Array.from({ length: totalPages }, (_, i) => (
            <motion.button key={i} onClick={() => goToPage(i)} className={`px-4 py-2 border-r border-gray-300 ${currentPage === i ? "text-[#2E3092] font-medium" : "text-gray-500 hover:bg-gray-100"} bg-white`}>
              {i + 1}
            </motion.button>
          ))}
          <motion.button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages - 1} className={`px-4 py-2 bg-white ${currentPage === totalPages - 1 ? "text-gray-400 cursor-not-allowed" : "text-[#2E3092]"}`}>
